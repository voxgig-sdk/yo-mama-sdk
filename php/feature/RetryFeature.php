...
class YoMamaRetryFeature extends YoMamaBaseFeature
{
    ...
    public function init(YoMamaContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YoMamaContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YoMamaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YoMamaContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

...
class YoMamaSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = YoMamaHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = YoMamaHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!YoMamaFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, YoMamaFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return YoMamaUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = YoMamaHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = YoMamaHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = YoMamaHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new YoMamaError($op . "_allow",
                "YoMamaSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = YoMamaHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new YoMamaError("graphql_error",
                "YoMamaSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetRandomJoke($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_random_joke === null) {
                $this->_get_random_joke = new GetRandomJokeEntity($this, null);
            }
            return $this->_get_random_joke;
        }
        return new GetRandomJokeEntity($this, $data);
    }

...
The PHP SDK for the YoMama API — an entity-oriented client using PHP conventions.
...
## Install
```bash
composer require voxgig-sdk/yo-mama
```
...
[$result, $err] = $client->YoMama()->load(["id" => "test01"]);
...
```php
$client = YoMamaSDK::test($testopts, $sdkopts);
```
...
The main class (`yomama_sdk.php`) exports the SDK class

...
class FtCtrl extends YoMamaControl
{
...
class FtClient
{
    ...
    private YoMamaUtility $utility;

    public function __construct(YoMamaUtility $utility, array $options)
    {
    ...
    public function get_utility(): YoMamaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YoMamaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YoMamaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YoMamaConfig::make_feature($name);
    ...
    private static function buildUrl(YoMamaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YoMamaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YoMamaResponse($fetched);
        }
    ...
    private function fail(YoMamaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YoMamaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

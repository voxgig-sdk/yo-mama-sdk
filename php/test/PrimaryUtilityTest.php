...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        YoMamaSDK $client,
        YoMamaUtility $utility
    ): YoMamaContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new YoMamaContext($ctxmap, null);
    ...
    private static function fixctx(YoMamaContext $ctx, YoMamaSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?YoMamaError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new YoMamaError($code, $msg);
    }
    ...
    {
        $hook_client = YoMamaSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends YoMamaBaseFeature {
            public $hook_fn = null;
            public function TestHook(YoMamaContext $ctx): void
            {
    ...
    {
        $init_client = YoMamaSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends YoMamaBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new YoMamaSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = YoMamaSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = YoMamaSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof YoMamaError) ? $result->err->msg : (string)$result->err;

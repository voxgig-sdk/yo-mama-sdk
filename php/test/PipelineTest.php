...
{
    private static function utility(): YoMamaUtility
    {
        return new YoMamaUtility();
    }
    ...
    private static function ctx(array $over = []): YoMamaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new YoMamaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new YoMamaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof YoMamaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new YoMamaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = YoMamaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new YoMamaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = YoMamaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new YoMamaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = YoMamaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new YoMamaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = YoMamaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new YoMamaResponse([]),
            'result' => new YoMamaResult([]),
        ]);
        [, $err] = YoMamaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new YoMamaSpec(['step' => 's']),
            'response' => new YoMamaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new YoMamaResult([]),
        ]);
        [, $err] = YoMamaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new YoMamaSpec(['step' => 's']),
            'response' => new YoMamaResponse(self::resp(200, ['v' => 1])),
            'result' => new YoMamaResult([]),
        ]);
        [, $err] = YoMamaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new YoMamaSpec(['step' => 's']),
            'response' => new YoMamaResponse(self::resp(200, ['v' => 2])),
            'result' => new YoMamaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        YoMamaMakeResponse::call($ctx);
    ...
    {
        $preset = new YoMamaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new YoMamaSpec([]),
            'response' => new YoMamaResponse([]),
            'result' => new YoMamaResult([]),
        ...
        [$got, $err] = YoMamaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new YoMamaResult([])]);
        [, $err] = YoMamaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new YoMamaSpec([]), 'result' => null]);
        [, $err] = YoMamaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new YoMamaSpec(['step' => 's']),
            'result' => new YoMamaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new YoMamaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = YoMamaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new YoMamaSpec(['step' => 's']),
            'result' => new YoMamaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new YoMamaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = YoMamaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new YoMamaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new YoMamaSpec([]),
            'result' => new YoMamaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = YoMamaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = YoMamaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new YoMamaError('boom', 'boom');
        $utility->fetcher = function (YoMamaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new YoMamaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = YoMamaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (YoMamaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = YoMamaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(YoMamaResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (YoMamaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new YoMamaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = YoMamaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new YoMamaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new YoMamaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = YoMamaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = YoMamaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = YoMamaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new YoMamaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, YoMamaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new YoMamaResult(['ok' => false])]);
        $this->expectException(YoMamaError::class);
        YoMamaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new YoMamaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', YoMamaMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new YoMamaBaseFeature();
        $b = new YoMamaBaseFeature();
        YoMamaFeatureAdd::call($ctx, $a);
        YoMamaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): YoMamaBaseFeature
    {
        $f = new YoMamaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        YoMamaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): YoMamaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new YoMamaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = YoMamaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new YoMamaResponse(['headers' => null]),
            'result' => new YoMamaResult([]),
        ]);
        YoMamaResultHeaders::call($ctx);

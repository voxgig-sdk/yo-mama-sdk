...
class YoMamaMakeError
{
    public static function call(?YoMamaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new YoMamaContext([], null);
        }
        $op = $ctx->op ?? new YoMamaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new YoMamaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof YoMamaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new YoMamaError('', $msg, $ctx);

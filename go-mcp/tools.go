...
type Args struct {
	Entity string         `json:"entity" jsonschema:"category | get_random_joke | joke"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.YoMamaSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "yo-mama_list",
		Description: "List records from YoMama. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "yo-mama_load",
		Description: "Load a single record from YoMama. " +
...
func runOp(client *sdk.YoMamaSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.YoMamaSDK, name string) (sdk.YoMamaEntity, error) {
	switch strings.ToLower(name) {
	case "category":
		return client.Category(nil), nil
	case "get_random_joke":
		return client.GetRandomJoke(nil), nil
	case "joke":

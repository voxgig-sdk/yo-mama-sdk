...
class GetRandomJokeEntity
  ...
  def make
    opts = @_entopts.dup
    GetRandomJokeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YoMamaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = YoMamaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.GetRandomJoke.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = YoMamaHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

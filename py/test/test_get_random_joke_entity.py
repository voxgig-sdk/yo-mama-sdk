    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YOMAMA_TEST_GET_RANDOM_JOKE_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_random_joke_ref01_data_dt0_loaded, err = get_random_joke_ref01_ent.load(get_random_joke_ref01_match_dt0, None)
        assert err is None
...
def _get_random_joke_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("YOMAMA_TEST_GET_RANDOM_JOKE_ENTID"))
    ...
    _live = env.get("YOMAMA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("YOMAMA_TEST_EXPLAIN") == "TRUE",
        "live": _live,

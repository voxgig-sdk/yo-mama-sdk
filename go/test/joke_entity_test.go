...
func TestJokeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Joke(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Joke(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set YO_MAMA_TEST_JOKE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		jokeRef01Match := map[string]any{}

		jokeRef01ListResult, err := jokeRef01Ent.List(jokeRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, jokeRef01ListOk := jokeRef01ListResult.([]any)
		if !jokeRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", jokeRef01ListResult)
		}
...
func jokeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YO_MAMA_TEST_JOKE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YO_MAMA_TEST_JOKE_ENTID"])

// Command YoMama-cli is a CLI + REPL for the YoMama SDK,
...
const prompt = "yo-mama"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "category get_random_joke joke"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      YO_MAMA_APIKEY (api key), YO_MAMA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: YO_MAMA_APIKEY carries the API key and
	// YO_MAMA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("YO_MAMA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("YO_MAMA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewYoMamaSDK(opts)

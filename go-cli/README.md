...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yo-mama-cli)
make build
...
YO_MAMA_BASE=https://api.example.com ./yo-mama-cli list category

# 5. No arguments -> interactive REPL
./yo-mama-cli
yo-mama> list category
yo-mama> :quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yo-mama-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yo-mama-cli list category
   ```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yo-mama-cli`. |

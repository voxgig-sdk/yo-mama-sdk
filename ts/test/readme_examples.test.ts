...
//     (\`new YoMamaSDK(...)\` / \`YoMamaSDK.test(...)\` become
//     \`YoMamaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"category":{"test01":{"id":"test01"}},"get_random_joke":{"test01":{"id":"test01"}},"joke":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new YoMamaSDK(...)\` and any
// \`YoMamaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YoMamaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YoMamaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

# YoMama TypeScript SDK



The TypeScript SDK for the YoMama API — a type-safe, entity-oriented client with full async/await support.
...
| `GetRandomJoke(data?)` | `GetRandomJokeEntity` | Create a GetRandomJoke entity instance. |
| `Joke(data?)` | `JokeEntity` | Create a Joke entity instance. |
| `tester(testopts?, sdkopts?)` | `YoMamaSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `YoMamaSDK.test(testopts?, sdkopts?)` | `YoMamaSDK` | Create a test-mode client. |
...
| `client` | `client(): YoMamaSDK` | Return the parent SDK client. |
...
```ts
const get_random_joke = await client.GetRandomJoke().load()
```
